...
class User(BaseModel, db.Model):
    ...
    def check_passowrd(self, password):
        #参数1：加密密码
        #参数2：未加密的密码
        #返回值：将未加密的密码按照相同的加密方式再次加密，然后在比较，如果一致返回True，否则返回False
...
class Comment(BaseModel, db.Model):
    ...
    parent_id = db.Column(db.Integer, db.ForeignKey("info_comment.id"))  # 父评论id（自关联）
